...
class HDF5Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        start = self._idx[idx]
            ...
            self._idx[idx + 1]
            if idx < len(self._idx) - 1
            else len(self._images)
        )
        assert end - start >= 0
        ...
        video = torch.tensor(
            self._images[start : start + self.sequence_length]
        )
...
class VideoData(pl.LightningDataModule):
    ...
    def _dataset(self, train):
        Dataset = (
            VideoDataset if osp.isdir(self.args.data_path) else HDF5Dataset
        )
        dataset = Dataset(
            self.args.data_path,
